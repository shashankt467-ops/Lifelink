...
import { Mail, Lock, User, Eye, EyeOff, Cross, Shield } from 'lucide-react';
...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!agree) { toast.error('Please accept the terms of service'); return; }
    setLoading(true);
    try {
      await register(form.email, form.password);
      toast.success('Account created! Welcome to Anti Gravity 🎉');
      navigate('/dashboard');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--bg-primary)' }}>
      ...
      <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}
        ...
        <h2 className="font-display font-bold text-3xl text-white mb-4">Join 50,000+ patients<br />using Anti Gravity</h2>
        <p style={{ color: 'rgba(255,255,255,0.6)', lineHeight: 1.7, fontSize: 15, marginBottom: 32 }}>
          Get instant access to hospitals, doctors, ambulances, and AI-powered health guidance.
        </p>
        {[
          '🏥 Find hospitals near you in seconds',
          '🤖 Get AI-powered health recommendations',
          '🚑 Request ambulances with one tap',
          '👨‍⚕️ Book specialist appointments instantly',
        ].map(f => (
          <p key={f} style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, marginBottom: 12 }}>{f}</p>
        ))}
      </motion.div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 overflow-y-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          ...
          <div className="lg:hidden flex items-center justify-center gap-2 mb-8">
            <div className="w-9 h-9 rounded-xl gradient-primary flex items-center justify-center"><Cross size={18} color="white" /></div>
            <span className="font-display font-bold text-xl text-gradient">AntiGravity</span>
          </div>

          <h1 className="font-display font-bold text-3xl mb-2" style={{ color: 'var(--text-primary)' }}>Create account</h1>
          <p style={{ color: 'var(--text-muted)', marginBottom: 28, fontSize: 15 }}>Join Anti Gravity for free today</p>
          ...
          <form onSubmit={handleRegister}>
            ...
            <div className="mb-4">
              ...
              <div className="input-group">
                ...
                <input value={form.name} onChange={set('name')} className="input" placeholder="Arjun Mehta" />
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="input-group">
                ...
                <input type="email" value={form.email} onChange={set('email')} className="input" placeholder="your@email.com" />
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="input-group" style={{ position: 'relative' }}>
                ...
                <input type={showPass ? 'text' : 'password'} value={form.password} onChange={set('password')} className="input" placeholder="Min. 6 characters" style={{ paddingRight: 44 }} />
              ...
              {form.password && (
                <div className="mt-2">
                  <div className="flex gap-1 mb-1">
                    {[0,1,2,3].map(i => (
                      <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i < strength ? strengthColors[strength - 1] : 'var(--border)', transition: 'all 0.3s' }} />
                    ))}
                  </div>
            ...
            <div className="mb-5">
              ...
              <div className="input-group">
                <Lock size={16} className="input-icon" />
                <input type="password" value={form.confirm} onChange={set('confirm')} className="input" placeholder="Repeat password"
                  style={{ borderColor: form.confirm && form.confirm !== form.password ? '#ef4444' : undefined }} />
              </div>
              {form.confirm && form.confirm !== form.password && <p style={{ fontSize: 12, color: '#ef4444', marginTop: 4 }}>Passwords don't match</p>}
            </div>
